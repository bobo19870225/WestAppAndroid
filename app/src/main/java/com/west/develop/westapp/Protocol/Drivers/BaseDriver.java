...
public abstract class BaseDriver extends BaseCMD {
    private static final int DEFAULT_PACK_MAX_SIZE = 260;
     ...
     * 应用程序下载地址
     */
    static final int START_ADDR_APPLICATION = 0x10000;
    /**
     * 固件程序下载地址
     */
    static final int START_ADDR_SYSTEM = 0x00000;
    byte[] RUNPack = new byte[DEFAULT_PACK_MAX_SIZE];
    byte[] WUNPack = new byte[DEFAULT_PACK_MAX_SIZE];
    TIOPack pack = new TIOPack();
    protected static Context mContext;
    private BaseSerialPort mPort;
     ...
     * 计算长包 CRC 校验值
     */
    private void LongPackCRC(int length) {
        WUNPack[length + 3] = 0;
        WUNPack[length + 4] = 0;
        int sum = 0;
     ...
     * 发送长包
     */
    boolean SendLongPack(TIOPack pack) {
     ...
     * 检查信息包中的CRC,正确是返回true
     */
    boolean CheckLongPackCRC(byte TempV, byte lenByt, int packlong) {
...
//        int tempCyc;
     ...
     * }dataBits = 5;
     */
    byte CheckCOMCHK(byte rev, byte chk) {
        return (byte) (~rev - (chk & 0x0F));
    }

    byte RevertCOMCHK(byte res, byte chk) {
        return (byte) ~res;
    }
    ...
    public int CpyByte(byte[] from, int offset, byte[] dest) {
        ...
        if (len >= 0) System.arraycopy(from, offset, dest, 0, len);
        return len;
    }
